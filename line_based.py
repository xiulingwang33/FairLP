...
for METHOD in METHODS:
    ...
    g_train_sampled = g_copy
    for node in g_train_sampled.nodes():
        for nbr in sorted(g_train_sampled.neighbors(node)):
            g_train_sampled[node][nbr]['weight'] = 1

    res_dir = './results'


    train_adj = nx.adjacency_matrix(g_train_sampled, weight='weight')
    train_tuple = (train_adj, features)
    with open('%s/train-%s-adj-feat.pkl' % (res_dir, DATA), "wb") as f:
        pickle.dump(train_tuple, f)


    graph_file = '%s/train-%s-adj-feat.pkl' % (res_dir,  DATA)
    n2v_scores, val_edge_labels, val_preds, test_edge_labels, test_preds = line.line.LINE(g_copy, train_test_split,graph_file, DATA, METHOD)
